...
public class LocationWs {
    ...
    @MessageMapping("/hook-last-location-by-work-order")
    @SendTo("/ws-response/get-last-location-by-work-order")
    public String getLastLocationByWorkOrder(String receiver) {
        Gson gson = new GsonBuilder().create();
        Request request = gson.fromJson(receiver,Request.class);
        return gson.toJson(new LocationUtl().getLastLocationByWorkOrder(entityManagerFactory,request));
    }
